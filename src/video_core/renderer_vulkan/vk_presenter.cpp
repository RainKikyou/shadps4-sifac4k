...
Frame* Presenter::PrepareFrame(const Libraries::VideoOut::BufferAttributeGroup& attribute,
    ...
    // Custom 4K mod: always use FSR EASU when upscaling the game image.
    if (image_size.width < frame->width && image_size.height < frame->height) {
...
void Presenter::SetExpectedGameSize(s32 width, s32 height) {
    // Custom 4K mod: lock the offscreen output to 3840x2160 once internal >= 4K.
    if (Config::getInternalScreenWidth() >= 3840 && Config::getInternalScreenHeight() >= 2160) {
        expected_frame_width = 3840;
        expected_frame_height = 2160;
        return;
    }
